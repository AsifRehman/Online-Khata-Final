...
generatePdfReport(context, String partName, List<LedgerModel> list,
  ...
  try {
    ...
    for (int i = list.length - 1; i >= 0; i--) {
      ...
      totalBal =
          isKyNotNull(list[i].debit) ? totalBal + list[i].debit : totalBal;
      totalBal =
    ...
    if (Platform.isAndroid) {
      ...
      if (int.parse(release) >= 11) {
        ...
        saveDataForAndroid11AndAbove(
            appDocDir, formattedDate, pdf, viewOrDownload, context);
      } else {
        //For Android 10 and below
        String folderName = "Online khata Reports";
        var dirPath = "storage/emulated/0/$folderName";
        dir = await new Directory(dirPath);
        saveData(dir, formattedDate, pdf, viewOrDownload, context);
      }
...
void saveData(Directory dir, String formattedDate, Document pdf,
    String viewOrDownload, context) async {
  if (!await dir.exists()) {
    await dir.create(recursive: true);
  }
  if (await dir.exists()) {
    ...
    await file.writeAsBytes(pdf.save()).then((value) {
      if (viewOrDownload == "from_share") {
        Share.shareFiles(['${dir.path}/Report_$formattedDate.pdf'], text: '');
      } else {
        material.Navigator.of(context).push(
          material.MaterialPageRoute(
            builder: (_) => PdfViewerPage(path: filePath, fileUri: file),
...
void saveDataForAndroid11AndAbove(Directory dir, String formattedDate,
    Document pdf, String viewOrDownload, context) async {
  final String filePath = dir.path + '/Report_$formattedDate.pdf';
  final File file = File(filePath);
